...
public class ChildrenController {
    ...
    public Reslt insert(Children children){

        int insert = childrenService.insert(children);
        log.info("children="+insert);
        if (insert>0){
            return new Reslt(200,"增加成功",insert);
        }
        else {
            return new Reslt(404,"失败",null);
        }
    }

    @GetMapping(value = "/children/getById/{id}")
    public Reslt getById(@PathVariable("id") long id){
        Children children = childrenService.getById(id);
        log.info("children="+children);
        if (children != null){
            return new Reslt(200,"查询成功",children);
        }
        else {
            return new Reslt(404,"查询失败，查询id="+id,null);
        }
    }
}
